...
class BillAckmanStrategy:
    ...
    def analyze_business_quality(self, metrics: List[Any], financial_line_items: List[Any]) -> Dict[str, Any]:
        ...
        revenues = [
            getattr(item, "revenue", None) or item.get("revenue")
            for item in financial_line_items
            if getattr(item, "revenue", None) or item.get("revenue") is not None
        ]
        if len(revenues) >= 2:
        ...
        op_margins = [
            getattr(item, "operating_margin", None) or item.get("operating_margin")
            for item in financial_line_items
            if getattr(item, "operating_margin", None) or item.get("operating_margin") is not None
        ]
        if op_margins:
        ...
        fcf_values = [
            getattr(item, "free_cash_flow", None) or item.get("free_cash_flow")
            for item in financial_line_items
            if getattr(item, "free_cash_flow", None) or item.get("free_cash_flow") is not None
        ]
        if fcf_values:
    ...
    def analyze_financial_discipline(self, metrics: List[Any], financial_line_items: List[Any]) -> Dict[str, Any]:
        ...
        de_list = [
            getattr(item, "debt_to_equity", None) or item.get("debt_to_equity")
            for item in financial_line_items
            if getattr(item, "debt_to_equity", None) or item.get("debt_to_equity") is not None
        ]
        if de_list:
        ...
        dividends = [
            getattr(item, "dividends_and_other_cash_distributions", None) or item.get("dividends_and_other_cash_distributions")
            for item in financial_line_items
            if getattr(item, "dividends_and_other_cash_distributions", None) or item.get("dividends_and_other_cash_distributions") is not None
        ]
        if dividends:
        ...
        shares = [
            getattr(item, "outstanding_shares", None) or item.get("outstanding_shares")
            for item in financial_line_items
            if getattr(item, "outstanding_shares", None) or item.get("outstanding_shares") is not None
        ]
        if len(shares) >= 2:
    ...
    def analyze_stock(self, ticker: str, end_date: str) -> Optional[AgentStateData]:
        ...
        ackman_output = self.generate_ackman_output(ticker, analysis_data)

...
class BillAckmanStrategy:
    ...
    def get_financial_data(self, ticker: str, end_date: str) -> Optional[Dict[str, float]]:
        """Fetches key financial metrics and owner earnings data."""
        metrics_list = self.api_client.get_financial_metrics(ticker, end_date)
        if not metrics_list:
            return None

        financial_line_items = self.api_client.search_line_items(ticker, end_date, ["netIncome", "totalDebt", "freeCashFlow"])
    ...
    def analyze_stock(self, ticker: str, end_date: str) -> Optional[AgentStateData]:
        """Evaluates a stock based on Ackman's investment strategy and potential catalysts."""
        ...
        # Ackman Criteria: High Growth, Positive Cash Flow, Manageable Debt
        is_cash_flow_positive = financials["free_cash_flow"] > 0
        is_high_growth = financials["revenue_growth"] and financials["earnings_growth"] and financials["revenue_growth"] > 10 and financials["earnings_growth"] > 10
